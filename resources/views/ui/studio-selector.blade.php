    ...
    x-data="{
        ...
        selectStudio(studioId, studioName) {
            ...
            window.dispatchEvent(new CustomEvent('notify', {
                detail: {
                    type: 'success',
                    title: @js((string) __('ui::studio_selector.messages.studio_selected_title')),
                    body: @js((string) __('ui::studio_selector.messages.studio_selected_body')).replace(':studio_name', studioName)
                }
            }));
        }
    }"
...
>
    ...
    <div class="text-center">
        <h3 class="text-lg font-medium text-gray-900 dark:text-gray-100">
            {{ (string) __('ui::studio_selector.studio_list.title') }}
        </h3>
        <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">
            {{ (string) __('ui::studio_selector.studio_list.subtitle') }}
        </p>
    </div>

    @if($studios && $studios->count()/** @phpstan-ignore method.nonObject */ > 0)
        {{-- Contatore studi trovati --}}
        <div class="text-center">
            <p class="text-sm font-medium text-blue-600 dark:text-blue-400">
                {{ (string) __('ui::studio_selector.studio_list.count_found', ['count' => $studios->count()/** @phpstan-ignore method.nonObject */]) }}
            </p>
        </div>
                        ...
                        <div class="flex-1">
                            ...
                            <div class="mt-2">
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-300">
                                    {{ (string) __('ui::studio_selector.studio_list.active_label') }}
                                </span>
                            </div>
                        </div>
                    ...
                    <div class="pt-4 border-t border-gray-200 dark:border-gray-700">
                        ...
                        >
                            <span x-show="selectedStudioId == {{ $studio->id }}">
                                {{ (string) __('ui::studio_selector.studio_list.actions.selected') }}
                            </span>
                            <span x-show="selectedStudioId != {{ $studio->id }}">
                                {{ (string) __('ui::studio_selector.studio_list.actions.select') }}
                            </span>
                        </button>
                    </div>
                </div>
    ...
    @else
        ...
        <div class="text-center py-12">
            ...
            <h3 class="mt-2 text-sm font-medium text-gray-900 dark:text-gray-100">
                {{ (string) __('ui::studio_selector.studio_list.empty_state.title') }}
            </h3>
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                {{ (string) __('ui::studio_selector.studio_list.empty_state.description') }}
            </p>
        </div>
    @endif
</div> 